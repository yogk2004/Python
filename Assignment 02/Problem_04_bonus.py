...
while k<6:
    import requests
    code_return=0
    while code_return!=200:
        word_guess=input().lower()
        while len(word_guess)!=5:
            print("Please give a '5 letter' word.")
            word_guess=input().lower()
        word_resp= requests.get("https://api.dictionaryapi.dev/api/v2/entries/en/"+word_guess)
        code_return=word_resp.status_code
        if code_return==404:
            print("Please give a valid-word with 5 letters only.")
    k+=1
    sam_pos_words=[]
    dif_pos_words=[]
    for i in range(5):
