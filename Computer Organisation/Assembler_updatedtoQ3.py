...
def Def_Error(code_org):
    ...
    list_inst=["add","sub","mul","xor","or","and","mov","rs","ld","ls","div","not","cmp","Id","st","jmp","jlt","jgt","je","hlt","var","addf","subf","movf"]
    ...
    for list_line in code_org:
        if list_line!=[] and list_line!=["hlt"]:
            if (list_line[0]=="rs" or list_line[0]=="ls" or list_line[0]=="mov" or list_line[0]=="movf"):
                decimal_Flag=0
                index=0
                for digit in list_line[2]:
                    if digit==".":
                        decimal_Flag=1
                        break
                    else:
                ...
                if decimal_Flag==0:
                    ...
                    if list_line[2][0]=="$":
                        num=float(list_line[2][1:])
                        if num<0 or num>31.5:
                            line_num=code_org.index(list_line)+1
                            Flag.append(["Illegal use of immediate value (Floating point number) at",line_num])

            if len(list_line)>1:
                if (list_line[1]=="rs" or list_line[1]=="ls" or list_line[1]=="mov" or list_line[1]=="movf"):
                    decimal_Flag=0
                    index=0
                    for digit in list_line[3]:
                        if digit==".":
                            decimal_Flag=1
                            break
                        else:
...
def printing_machinecode(assembly_code,list_var_add,list_label_add):
    for list_line in assembly_code:
        #Type A Instructions:
        if list_line[0]=="add" or list_line[0]=="sub" or list_line[0]=="mul" or list_line[0]=="xor" or list_line[0]=="or" or list_line[0]=="and" or list_line[0]=="addf" or list_line=="subf":
        ...
        #Type B Instructions:
        if list_line[0]=="mov" or list_line[0]=="rs" or list_line[0]=="ls" or list_line[0]=="movf":
...
list_line=[]
list_org=[]
""" file1 = open("Assembler_input.txt", "r")
for i in file1:
    list_line.append(i)
file1.close() """
...
for i in Def_Error(list_org):
    if i[0]=="Space between label and colon at":
        ...
        for j in i:
            print(str(j),end=" ")
        print("")
    if i[0]=="Illegal use of immediate value (Floating point number) at":
        flag_error="Yes"
        for j in i:
            print(str(j),end=" ")
        print("")
